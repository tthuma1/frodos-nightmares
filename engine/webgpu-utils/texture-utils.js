import { isTypedArray } from "./typed-arrays.js"
import { generateMipmap, numMipLevels } from "./generate-mipmap.js"
 ...
 */
export function getSizeForMipFromTexture(texture, mipLevel) {
  return [texture.width, texture.height, texture.depthOrArrayLayers].map(v =>
    Math.max(1, Math.floor(v / 2 ** mipLevel))
  )
}
 ...
 */
export function copySourcesToTexture(device, texture, sources, options = {}) {
  let tempTexture
 ...
 * to a texture and then optionally generates mip levels
 */
export function copySourceToTexture(device, texture, source, options = {}) {
 ...
 */
export function getSizeFromSource(source, options) {
  if (source instanceof HTMLVideoElement) {
    ...
    const data =
      isTypedArray(source) || Array.isArray(source) ? source : source.data
...
export async function loadImageBitmap(url, options = {}) {
  ...
  const opt = {
    ...options,
    ...(options.colorSpaceConversion !== undefined && {
